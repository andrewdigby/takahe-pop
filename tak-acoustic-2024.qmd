...
library(tidyverse)
packages <- c(
"readxl", "lubridate", "purrr", "conflicted",
"ggplot2", "ggpubr",
"yardstick", # for contingency stats
"binom" # confidence intervals
)
...
Proportional accuracy by block using weighted (bird-level) accuracy: sum(birds_responded) / sum(birds_surveyed) rather than mean(birds_responded / birds_surveyed)

```{r}
#| label: prop_acc_block_weight
#| code-summary: "Proportional accuracy by block using weighted accuracy"
#| fig-height: 8



ac2024_accuracy_wt <- ac2024_max %>%
  mutate(
  ...
  group_by(block) %>%
  summarise(
    birds_responded_total = sum(birds_responded),
    birds_surveyed_total = sum(birds_surveyed),
    accuracy = birds_responded_total / birds_surveyed_total,
    .groups = "drop"
  ) %>%
  rowwise() %>%
  mutate(
    ci = list(
      binom.confint(
        birds_responded_total,
        birds_surveyed_total,
        method = "wilson"
      )
    ),
    lower_ci = ci$lower,
...
ggplot(ac2024_accuracy_wt, aes(x = block, y = accuracy)) +
...
geom_text(
  aes(
    label = paste0("Survey n = ", birds_surveyed_total),
    y = pmin(upper_ci + 0.04, 1.08)
  ),
  size = 3
) +
  labs(
    title = "Weighted Accuracy by Block",
    x = "Block",
    y = "Detection Accuracy (Birds Responded / Birds Surveyed)"
  ) +
